...
def listar_empleados():
    return jsonify({"empleados": employee_service.listar()}), 200
...
def crear_empleado():
    ...
    requeridos = ["codigo", "nombre_completo", "cargo", "usuario", "password"]
    ...
    try:
        ...
        return jsonify({"empleado": empleado}), 201
...
def actualizar_empleado(codigo):
    data = request.get_json(silent=True) or {}
    try:
        empleado = employee_service.actualizar(codigo.upper(), **data)
        return jsonify({"empleado": empleado}), 200
    except ValueError as e:
...
def desactivar_empleado(codigo):
    try:
        ...
        return jsonify({"empleado": empleado, "mensaje": "Empleado desactivado."}), 200
    except ValueError as e:
        return jsonify({"error": str(e)}), 404
